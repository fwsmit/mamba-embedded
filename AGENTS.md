...
To inspect the MCU results as tables (latency, full per-context memory profile)
instead of plots, run `tools/mambalite_stats.py`: it parses
every `experiments/mambalite-micro/*.output` and prints a summary table plus
the full memory profile for each model. Path and `--filter` arguments optional:

```bash
python tools/mambalite_stats.py                          # all runs in experiments/mambalite-micro
python tools/mambalite_stats.py --filter har             # only HAR runs
python tools/mambalite_stats.py --filter _int16          # only int16 quantizations
python tools/mambalite_stats.py path/to/output_dir       # custom directory
```
...
conda run -n torch-pascal python -m train.plot_arch_search --plot pareto --par-acc-top auto config/kws/arch-mamba1-kws-2.yaml
...
conda run -n torch-pascal python -m train.plot_arch_search --plot confusion --trial 18 config/har/arch-mamba1-har-bidir-mul.yaml
```
...
|----------------|-------------|
| `pareto` | Compares Pareto fronts of multiple experiments on PC latency vs accuracy. The y-axis is capped at 100% by default; pass `--par-acc-top auto` to frame the y-axis around the Pareto front points instead (min/max with 10% padding, snapped to the 2% tick grid) |
...
| `param_accuracy` | Scatter plot of nr of parameters (x, logarithmic axis) vs accuracy (y) from `results.json`, with the N most parameter-efficient models selected from the combined Pareto front (max accuracy, min parameters) across ALL studies shown as one "Ours" trendline. The models are selected using the **validation-set** accuracy but plotted at their **test-set** accuracy. Use `--n-models N` (default 10) to control how many models are selected in total (not per study). Use `--min-val-acc N` to restrict selection to models scoring strictly above N% on the validation set first. Use `--size` (32 float / 16 / 8 bits, default 8) with `--quantization` (`no` / `percent` / `tqt`, default `percent`) to pick the accuracy metric: `32+no` → float, `16+percent` → int16 PTQ, `8+percent` → int8 PTQ, `8+tqt` → int8 KL-TQT. For HAR and KWS studies, reference points from the literature (`har-numbers-literature.md` / `kws-numbers-literature.md`, see `HAR_LITERATURE_POINTS` / `KWS_LITERATURE_POINTS` in `train/plot_types/param_accuracy.py`) use one marker shape per model family, with variants connected by lines and all paper/model names shown in the single legend below each graph. F1-scores use open markers. The KWS LMU models report their size in kbits, which is converted to parameters assuming int8 storage (kbits / 8) and noted as "model size, int8 params". |
...
| `confusion` | Confusion matrix of a **specific trial's MCU predictions** on the validation set: reads `experiments/<study>/predictions_trial_<N>.json` (per-class predictions parsed from the firmware output) against the matching ground truth (validation labels and class names, loaded exactly as the quantization pipeline builds the validation set: the UCI HAR 80/20 seed-42 val split, or the KWS val pkl from `~/Datasets`/`kws_dataset/`). Row-normalised heatmap with per-cell counts (count + row % for small matrices), class names on both axes and the overall validation accuracy annotated in the corner. Requires `--trial N` and exactly one config. Exported at 300 DPI PNG plus PDF |
| `importance` | Two figures of hyperparameter importance (one per objective: Accuracy/Latency), each with two stacked heatmaps (one per dataset, HAR on top, KWS below, dataset name on top of each heatmap; column labels only on the bottom one): rows are the studies, columns are the hyperparameters, cells coloured by importance with value labels. Importance values are computed with optuna's PedAnova evaluator on the raw objective value — the same computation as optuna-dashboard — so the numbers match the dashboard. (PedAnova measures importance for *low* target values, so the Accuracy heatmap shows which parameters drive accuracy *down*, e.g. lr causing diverged training.) Rows are ordered by dataset (HAR first) then architecture variant (single → bidir add → bidir mul); the 0–1 colour scale is shared across all heatmaps. Pass all configs at once (e.g. `config/har/* config/kws/*`) to get the full 6-study grid. |
...
| `mambalite` | Bar charts comparing this work's Mamba-Lite micro models against the published Mamba-Lite micro reference, per dataset (HAR and KWS). Three metrics are compared, each as one bar chart per dataset: average single-inference latency (ms), memory footprint (total KB; a hierarchical stacked bar per experiment using the full memory profile: segments for parameters in flash and their RAM copy (both blue — "the copy shown the same way"), variables (purple), the model/fbs_model non-parameter part (green) and others (grey), whose sum is RAM + PSRAM + flash — while the Mamba-Lite reference only publishes a combined RAM + flash total, so its bar stays a single hatched bar) and flash model storage (bytes; converted from the KB flash total, with this work's bars stacked and coloured by content — blue parameters on the bottom, green "other" (e.g. program code) on top — while the Mamba-Lite reference only publishes a flash total, so its bar stays a single hatched bar). Successful runs are parsed directly from `experiments/mambalite-micro/*.output` (a run counts if the log has an `INFERENCE_OK` line, a latency figure and the memory-summary total row); failed runs are skipped. The Mamba-Lite reference bar is hatched. Raster PNG at 300 DPI plus PDF |
